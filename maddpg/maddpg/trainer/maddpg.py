...
def discount_with_dones(rewards, dones, gamma):
    discounted = []
    r = 0
    for reward, done in zip(rewards[::-1], dones[::-1]): # 翻转 What's the point ...
        r = reward + gamma*r
        r = r*(1.-done) # 把done转化为0, 1
        discounted.append(r)
    return discounted[::-1]

# polyak averaging，用于软更新
def make_update_exp(vals, target_vals):
    polyak = 1.0 - 1e-2
    expression = []
    for var, var_target in zip(sorted(vals, key=lambda v: v.name), sorted(target_vals, key=lambda v: v.name)):
        expression.append(var_target.assign(polyak * var_target + (1.0-polyak) * var))
    expression = tf.group(*expression) # *inputs是0个或者多个用于组合tensor
...
def p_train(make_obs_ph_n, act_space_n, p_index, p_func, q_func, optimizer, grad_norm_clipping=None, local_q_func=False, num_units=64, scope="trainer", reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # 将actor（policy侧网络）生成的动作和状态生成Q critic
        ...
        # 得到critic侧训练函数train，输入是状态和动作，输出是loss
