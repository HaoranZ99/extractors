...
class Scenario(BaseScenario):
    ...
    def adversary_reward(self, agent, world):
        ...
        neutrals_agents = self.neutrals(world)
        good_rew += sum(a.state.fear * 0.1 if a.state.health >= -200 else 0 for a in neutrals_agents)
    ...
    def neutral_reward(self, agent, world):
        pos_rew = max(agent.state.health, 0) * 0.1
        return pos_rew
    ...
    def done(self, agent, world):
        ...
        if agent.adversary:
            ...
            elif agent.state.anger > 80 and np.random.uniform() < 0.4:
                done = 5
        elif not agent.neutral:
            if agent.action == 1:
                done = 1
            elif agent.state.health < 40 and np.random.uniform() < 0.4:
                done = 4
            elif agent.action == 2 and np.random.uniform() < 0.1:
                done = 6
        else:
            if agent.action == 2 and np.random.uniform() < 0.1:
                done = 7
        return done
